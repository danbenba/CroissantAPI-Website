...
export interface Game {
  ...
  discount_price?: number // Prix promotionnel
  // Données studio (comme Croissant)
  owner_id?: string
  studio?: {
    id: string
    username: string
              ...
              <div className="flex flex-col gap-1 px-5 py-2">
                ...
                {game.studio && (
                  <div className="z-10 flex items-center gap-2 mb-1">
                    <Image
                      src={game.studio.avatar || `/avatar/${game.studio.id}`}
                      alt={game.studio.username}
                      className="w-5 h-5 rounded-full"
                      fallbackSrc="/storage/icons/default_profile.png"
                    />
                    <span className="text-sm text-default-500">{game.studio.username}</span>
                    {game.studio.certification && (
                      <div className="w-4 h-4">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 256 256" className="text-blue-400">
                          <path d="M225.86,102.82c-3.77-3.94-7.67-8-9.14-13.23s-1.74-10.69-1.74-16.18,0-11,1.74-16.18,5.37-9.29,9.14-13.23a8,8,0,0,0,1.21-8.58A97.32,97.32,0,0,0,209.24,20.6a8,8,0,0,0-8.58,1.21c-3.94,3.77-8,7.67-13.23,9.14s-10.69,1.74-16.18,1.74-11,0-16.18-1.74-9.29-5.37-13.23-9.14a8,8,0,0,0-8.58-1.21A97.32,97.32,0,0,0,118.42,35.42a8,8,0,0,0,1.21,8.58c3.77,3.94,7.67,8,9.14,13.23s1.74,10.69,1.74,16.18,0,11-1.74,16.18-5.37,9.29-9.14,13.23a8,8,0,0,0-1.21,8.58,97.32,97.32,0,0,0,14.82,15.82,8,8,0,0,0,8.58-1.21c3.94-3.77,8-7.67,13.23-9.14s10.69-1.74,16.18-1.74,11,0,16.18,1.74,9.29,5.37,13.23,9.14a8,8,0,0,0,8.58,1.21,97.32,97.32,0,0,0,15.82-14.82A8,8,0,0,0,225.86,102.82Zm-52.2,6.84-24,24a8,8,0,0,1-11.32,0l-12-12a8,8,0,0,1,11.32-11.32L144,116.69l18.34-18.35a8,8,0,0,1,11.32,11.32Z"></path>
                        </svg>
                      </div>
                    )}
                  </div>
                      ...
                      {game.price === 0 || !game.price ? (
                        <>
                         ...
                         <div className="flex items-center gap-2">
                           <span className="text-[#ffd700] font-bold">{Math.floor(game.price)}</span>
                           <Image 
                           ...
                           {game.discount_price && (
                             <>
                               <span className="text-xs text-default-500 line-through ml-1">{Math.floor(game.discount_price)}</span>
                               <span className="text-xs bg-red-500 text-white px-1 rounded">-{Math.round((1 - game.discount_price / game.price) * 100)}%</span>
                             </>
                           )}
                         </div>
                        )}
