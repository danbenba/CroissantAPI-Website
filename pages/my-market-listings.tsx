    ...
    return (
        <div className="inventory-tooltip" style={{
            ...
            {listing.price !== undefined && (
                <div style={{ color: "#ffd700", fontSize: 12, marginBottom: 4 }}>
                    Purchase price: {listing.price} <img src="/assets/credit.png" alt="credits" style={{ width: 12, verticalAlign: "middle" }} />
                </div>
            )}
            {listing.metadata?._unique_id && (
                <div style={{ color: "#888", fontSize: 11, fontFamily: "monospace" }}>
                    Unique ID: {listing.metadata._unique_id}
                </div>
            )}
        </div>
    );
...
export default function MyMarketListingsPage() {
    ...
    const [tooltip, setTooltip] = useState<{ x: number, y: number, listing: EnrichedMarketListing } | null>(null);
    ...
    if (!user) return <div>You must be logged in to view your market listings.</div>;
