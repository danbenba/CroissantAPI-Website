...
function ProfileShop({
	...
	const customPrompt = async (
		...
		return new Promise<{ confirmed: boolean; amount?: number }>(
			(resolve) => {
				setPrompt({ message, resolve, maxAmount, amount: 1, item });
				setPromptOwnerUser(ownerUser);
			},
	...
	const handlePromptAmountChange = (
		...
		<div className="profile-shop-section">
			...
			{tooltip && (
				...
				>
					...
					<div className="shop-tooltip-desc">
...
function useProfileLogic(userId: string) {
	...
	const [giveCreditsError, setGiveCreditsError] = useState<string | null>(
		null,
	);
